"""
Code adapted from Carnall (2017)
...
Needed to add fill_errs argument
"""
...
def make_bins(wavs):
    ...
    of corresponding wavelength bins. (https://spectres.readthedocs.io/en/latest/)"""
...
def spectres(new_wavs, spec_wavs, spec_fluxes, spec_errs=None, fill_flux=None, fill_errs=None, bin_errs=True,
    ...
    for j in range(new_wavs.shape[0]):
        ...
        if (new_edges[j] < old_edges[0]) or (new_edges[j+1] > old_edges[-1]):
            new_fluxes[..., j] = fill_flux
            if spec_errs is not None:
                new_errs[..., j] = fill_errs
        ...
        # Otherwise multiply the first and last old bin widths by P_ij
            ...
            elif old_errs is not None and bin_errs is False:
                e_wid = old_widths[start:stop+1]*old_errs[..., start:stop+1]
                new_errs[..., j] = np.sum(e_wid, axis=-1)
                new_errs[..., j] /= np.sum(old_widths[start:stop+1])
