...
def main(args):
    ...
    if args.plot:
        ...
        plt.savefig(fig_dir.joinpath(f'{obs_name}_obs_{args.resolution}.png'))
    ...
    priors = {
        'stellar_labels': [
                              ...
                              GaussianLogPrior('logg', logg_mu, logg_sigma)
    ...
    for n in range(int(args.n_fits)):
        print(f'Beginning fit {n+1}/{args.n_fits} for {obs_name}')
        ...
        with torch.no_grad():
            stellar_labels0[:, 0] = teff_mu
            stellar_labels0[:, 1] = logg_mu
        rv0 = 'prefit'
        log_vmacro0 = torch.FloatTensor(1, 1).uniform_(-1.0, 1.3) if args.fit_vmacro else None
        log_vsini0 = torch.FloatTensor(1, 1).uniform_(-1.0, 1.0) if args.fit_vsini else None
        if args.fit_inst_res:
            inst_res0 = ensure_tensor(payne.model_res) if args.resolution == 'default' else ensure_tensor(
                int(args.resolution))
            inst_res0 = torch.min(
        ...
        np.savez(
            fits_dir.joinpath(f'{obs_name}_fit_{args.resolution}_{n + 1}.npz'),
            **optim_fit
        )
        ...
        if args.plot:
            ...
            for i in range(optimizer.n_obs_ord):
                ...
                plt.savefig(fig_dir.joinpath(f"{obs_name}_spec_{args.resolution}_{int(obs['ords'][i])}.png"))

            print(f'Completed Fit {n + 1}/{args.n_fits} for {obs_name}')
            del optimizer
            del optim_fit
            gc.collect()
