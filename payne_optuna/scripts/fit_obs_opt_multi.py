...
import torch
from payne_optuna.fitting import PayneStitchedEmulator, UniformLogPrior, GaussianLogPrior, FlatLogPrior
from payne_optuna.fitting import PayneEmulatorMulti, PayneOptimizerMulti
...
def main(args):
    ...
    for exp_fitting_configs in configs['paths']['exp_configs']:
        ...
        payne = PayneStitchedEmulator(
            models=models,
            cont_deg=exp_configs['fitting']['cont_deg'],
    ...
    ######## FIT SPECTRA ########
    #############################
    payne = PayneEmulatorMulti(
        emulators=[emulator for key, emulator in payne_dict.items()]
    )
    n_fits = int(configs['fitting']['n_fits'])
    for n in range(n_fits):
        print(f'Beginning fit {n + 1}/{n_fits} for {obs_name}')
        # Initialize Optimizer
        optimizer = PayneOptimizerMulti(
            emulator=payne,
        ...
        if not configs['fitting']['fit_vmicro']:
            with torch.no_grad():
                stellar_labels0[:, 2] = payne.scale_stellar_labels(
                    (2.478 - 0.325 * payne.unscale_stellar_labels(stellar_labels0)[:, 1]) * torch.ones(
                        payne.n_stellar_labels)
                )[2]
        rv0 = 'prefit'
        ...
        if configs['fitting']['fit_inst_res']:
            ...
            print(
                'Observation has default resolution that matches model resolution---no instrumental broadening necessary')
            inst_res0 = None
        else:
            raise RuntimeError("Resolution was parsed incorrectly---this should not happen.")
        # Begin Fit
        spec_key = 'spec' if resolution == "default" else 'conv_spec'
        errs_key = 'errs' if resolution == "default" else 'conv_errs'
        wave_key = 'wave' if resolution == "default" else 'conv_wave'
        mask_key = 'mask' if resolution == "default" else 'conv_mask'
        optimizer.fit(
            obs_flux=np.array([obs[spec_key] for key, obs in obs_dict.items()]),
            obs_errs=np.array([obs[errs_key] for key, obs in obs_dict.items()]),
            obs_wave=np.array([obs[wave_key] for key, obs in obs_dict.items()]),
            obs_mask=np.array([obs[mask_key] for key, obs in obs_dict.items()]),
            obs_blaz=None,
            params=dict(
                stellar_labels='fit',
                rv='fit',
        ...
        optim_fit = {
            ...
            'cont_coeffs': optimizer.cont_coeffs_tensor.detach(),
        ...
        np.savez(
           fits_dir.joinpath(f"{configs['name']}_{n + 1}.npz"),
           **optim_fit
        )
        ...
        if configs['output']['plot_conv']:
            ...
            n_panels = payne.n_cont_coeffs*payne.n_emulators + 2 + np.sum(
            ...
            for i in range(optimizer.n_stellar_labels):
                ax1.plot(
                    ...
                    label=optimizer.emulator.labels[i],
                    alpha=0.5
                )
            ...
            if optimizer.inst_res is not None:
                ...
                ax.legend()
                panel += 1
            cont_coeffs = torch.stack(optimizer.history['cont_coeffs']).detach().numpy()
            for o in range(optimizer.n_obs):
                for i in range(optimizer.n_cont_coeffs):
                    ax = plt.subplot(gs[panel, 0])
                    for j in range(optimizer.n_obs_ord):
                        ax.plot(cont_coeffs[:, o, i, j], alpha=0.5)
                    panel += 1
            plt.savefig(fig_dir.joinpath(f"{configs['name']}_{n + 1}.png"))
            plt.close('all')

        # Plot Fits
        if configs['output']['plot_fit']:
            print('Plotting Fits')
            best_model = optimizer.best_model[0].detach().numpy()
            best_model_errs = optimizer.best_model_errs[0].detach().numpy()
            obs_flux = optimizer.obs_flux.detach().numpy()
            obs_errs = optimizer.obs_errs.detach().numpy()
            chi2 = ((best_model - obs_flux) / (
                np.sqrt(best_model_errs ** 2 + obs_errs ** 2))) ** 2
            for i in range(optimizer.n_obs_ord):
                for o in range(optimizer.n_obs):
                    ...
                    #    fontsize=48)
                    obs_wave = optimizer.obs_wave.detach().numpy()[o, i]
                    obs_mask = optimizer.obs_mask.detach().numpy()[o, i]
                    ax1.scatter(
                        obs_wave[obs_mask],
                        obs_flux[o, i][obs_mask],
